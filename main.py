...
for i in range(6):
    ...
    plt.title(f'Model of Type {numerals[i]}')
    plt.xlabel('Epoch')
    plt.ylabel('Mean Classification Error Rate')
    plt.savefig(f'graphs/graph_{numerals[i]}.png')
    plt.close()

plt.figure()
for i in range(6):
    plt.plot(x, averages[i], label=f"type {numerals[i]}")
...
for i in range(6):
    ...
    plt.title(f'Focus Model of Type {numerals[i]}')
    plt.xlabel('Epoch')
    plt.ylabel('Mean Classification Error Rate')
    plt.savefig(f'focus_graphs/focus_graph_{numerals[i]}.png')
    plt.close()

plt.figure()
for i in range(6):
    plt.plot(x, averages_focus[i], label=f"type {numerals[i]}")
